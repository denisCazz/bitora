...
function calculateStrength(password: string): { score: number; label: string; color: string } {
  ...
  if (score <= 4) return { score: Math.min(score * 10, 50), label: 'Discreta', color: 'bg-gray-500' };
...
const cardClass = 'p-6 rounded-2xl bg-gray-50 dark:bg-zinc-900/50 border border-gray-200 dark:border-white/10';
const inputSmClass = 'w-20 rounded-lg border border-gray-300 dark:border-white/10 bg-white dark:bg-white/5 px-3 py-1.5 text-center text-gray-900 dark:text-white text-sm focus:border-gray-500 dark:focus:border-white/40 focus:outline-none';
const rowClass = 'flex items-center justify-between p-3 rounded-xl bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/[0.08] cursor-pointer transition-colors';
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className={cardClass}>
        <div className="flex items-center gap-3">
          <div className="flex-1 overflow-hidden">
            ...
            >
              {password || <span className="text-gray-400 dark:text-gray-500">Seleziona almeno un'opzione</span>}
            </p>
          </div>
        ...
        <div className="mt-4 space-y-2">
          <div className="flex justify-between items-center">
            ...
            <span className={`text-xs font-semibold ${
              strength.label === 'Fortissima' ? 'text-green-500 dark:text-green-400' :
              strength.label === 'Buona' ? 'text-yellow-500 dark:text-yellow-400' :
              strength.label === 'Discreta' ? 'text-gray-500' :
              strength.label === 'Debole' ? 'text-red-500 dark:text-red-400' : 'text-gray-400'
            }`}>{strength.label}</span>
          </div>
      ...
      <div className={`${cardClass} space-y-6`}>
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Lunghezza</label>
            <input
              type="number"
              min="4"
              max="128"
          ...
          <input type="range" min="4" max="128" value={length} onChange={(e) => setLength(Number(e.target.value))} className="w-full accent-gray-900 dark:accent-white" />
          ...
          {([
            ...
            <label key={key} className={rowClass}>
              <div>
                ...
                <span className="ml-2 text-xs text-gray-400 dark:text-gray-500 font-mono">{example}</span>
              </div>
              <button
        ...
        <details className="group">
          ...
          <div className="mt-4 space-y-4">
            <label className={rowClass}>
              <div>
                <span className="text-sm text-gray-900 dark:text-white">Escludi caratteri ambigui</span>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Escludi caratteri personalizzati</label>
              <input
                type="text"
                value={customExclude}
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Quantità password</label>
                <input
                  type="number"
                  min="1"
                  max="50"
      ...
      {passwords.length > 1 && (
        <div className={`${cardClass} space-y-3`}>
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Password generate ({passwords.length})</h3>
            <button
              onClick={copyAll}
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {passwords.map((pw, i) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/[0.08] group">
                <span className="text-xs text-gray-400 dark:text-gray-500 w-6">{i + 1}.</span>
                <span className="flex-1 font-mono text-sm text-gray-900 dark:text-white break-all select-all">{pw}</span>
                <button
      ...
      <div className="p-4 rounded-xl bg-gray-100 dark:bg-white/10 border border-gray-200 dark:border-white/20">
        <p className="text-sm text-gray-600 dark:text-gray-300">
          <strong>🔒 Sicurezza:</strong> Le password vengono generate interamente nel tuo browser usando l'API <code className="text-gray-800 dark:text-gray-200 bg-gray-200 dark:bg-white/10 px-1.5 py-0.5 rounded">crypto.getRandomValues()</code>. Nessun dato viene inviato al server.
        </p>
      </div>
    </div>
  );
